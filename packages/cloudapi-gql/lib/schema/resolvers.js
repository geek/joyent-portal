...
module.exports = internals.resolvers = {
  ...
  machine: async (args, request, context) => {
    return await Api.machines.get(args, request, context);
  },
  ...
  snapshots: async ({ name, machine }, request) => {
    ...
    return await Api.machines.snapshots.list({ id: machine }, request);
  },
...
/*
User: {
    keys: ({ login }, { name }) => internals.resolvers.Query.keys(null, { login, name })
  },
  Machine: {
    ...
    networks: ({ networks }) =>
      Promise.all(networks.map(id => internals.resolvers.Query.network(null, { id }))),

    // eslint-disable-next-line camelcase
    package: root => resolvers.Query.package(null, { name: root.package }),

    snapshots: ({ id }, { name }) =>
    internals.resolvers.Query.snapshots(null, { machine: id, name }),

    // eslint-disable-next-line camelcase
    firewall_rules: ({ id: machine }, { id }) =>
    internals.resolvers.Query.firewall_rules(null, { machine, id }),

    actions: ({ id }) => internals.resolvers.Query.actions(null, { machine: id })
  },
  Image: {
