...
module.exports = (method, args, request) => {
  ...
  const getAuthHeaders = util.promisify(_getAuthHeaders.bind(client));
...
module.exports.fetch = async (_pathname, reqOpts = {}) => {
  const method = (reqOpts.method || 'get').toUpperCase();
  const pathname = _path.call(client, _pathname.replace(/:login/, account));
